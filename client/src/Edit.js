...
export default function Edit({ res, images, onFinish }) {
	...
	const prevIndex = useCallback(() => setIndex(Math.max(index - 1, 0)), [
		index,
	])
	...
	const btns = (
		<div>
			<button onClick={prevIndex} disabled={index === 0}>
				Prev
			</button>
			<button onClick={() => setStage(1)}>Finish</button>
			<button onClick={nextIndex} disabled={index >= images.length - 1}>
				Next
			</button>
		</div>
	)

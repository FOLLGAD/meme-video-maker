...
export default function FileImage({
    ...
    const onCanvasMouseDown = useCallback(
        ...
        [setMouseDownAt, scale, canvasRef]
    )
    ...
    useEffect(() => {
        ...
        const drawOverlay = () => {
            ...
            // Draw reading blocks
            lineBlocks.forEach(
                ({ blocks, rect: { x, y, width, height } }, i) => {
    ...
    }, [lineBlocks, src, ctx, pipeline, highlight, scale])
    ...
    useEffect(() => {
        ...
        const onCanvasMouseUp = (event) => {
            ...
            if (
                Math.abs(mouseX - fromX) > 10 &&
                Math.abs(mouseY - fromY) > 10
            ) {
                // Mouse was dragged
                // add a reveal rect
                let arr: RawStage[] = []
                    ...
                    if (_foundIndexReversed !== -1) {
                        ...
                        elem.reads.push(
                            ...block.blocks.map((block) => ({
                                ...
                                rect: block.rect,
                                line:
                                    (elem.reads[elem.reads.length - 1].line ||
                                        0) + 1,
                            }))
                        )

...
export default function FileImage({ src, blocks, pipeline, setPipeline }) {
  ...
  const onCanvasMouseDown = (event) => {
    const [mouseX, mouseY] = getCursorPosition(canvasRef.current, event).map(
      (p) => p / scale
    );
  ...
  const onCanvasMouseUp = (event) => {
    ...
    if (found !== -1) {
      if (indexIsEnabled(found)) {
        setPipeline(
          pipeline.filter((s) => s.type !== "read" || s._index !== found)
        );
      } else if (shiftDown) {
        ...
        if (revInd !== -1 && !pipeline[ind].added.includes(found)) {
          const newText =
            pipeline[ind].text + " " + blocks[found].text.toLowerCase();
          updateStage(ind, {
            ...pipeline[ind],
            text: newText,
    ...
    <div style={{ display: "flex", userSelect: "none" }}>
      ...
      <div className="card">
        <div style={{ marginBottom: 5 }}>
          ...
          <button onClick={() => addStage({ type: "pause", secs: shortPause })}>
            Short pause
          </button>
          <button
            onClick={() =>

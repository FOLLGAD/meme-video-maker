...
const koaBody = koaMultiBody({ multipart: true, formidable: { uploadDir: uploadDir.path } })
...
async function getFile(s3_key: string): Promise<string> {
    if (cache.has(s3_key) && existsSync(cache.get(s3_key)!.path)) {
        ...
        return await new Promise((res, rej) => s3.getObject({
            Bucket: FilesBucket,
            Key: s3_key,
        }, async (err, data) => {
...
const generateName = () => {
    const now = new Date()
    const num = parseInt((now.getFullYear() - 2000).toString().padStart(2, "0")
...
async function makeVid(rawSet, enabled, images) {
    const settings = {
        ...
        transition: rawSet.transition ? await getFile(rawSet.transition) : undefined,
        outro: rawSet.outro ? await getFile(rawSet.outro) : undefined,
        song: rawSet.song ? await getFile(rawSet.song) : undefined,
        voice: rawSet.voice,
    }

    return await makeVids(enabled, images.map(i => i.image), settings)
        .then(vid => {
...
router
    ...
    .get('/files', async ctx => {
        const { Contents }: ListObjectsOutput = await new Promise((res, rej) => {
            s3.listObjects({
                Bucket: FilesBucket,
                MaxKeys: 100,
            }, (err, data) => {
    ...
    .post('/themes', bodyParser, async ctx => {
        ...
        await new Promise((res, rej) => dynamodb.putItem({
            Item,
            TableName: dbThemeName,
        }, (err, data) => err ? rej(err) : res(data)))
